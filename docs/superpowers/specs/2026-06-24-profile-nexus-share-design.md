...
**涵蓋（in scope）**
- Profile 建立／匯出時反查並夾帶每個 mod 的 Nexus 來源
...
- 前端**建立 profile 時**（`handleCreateProfile`）即呼叫此 IPC，把 `nexusSources` 存進 profile —— 趁所有 mod 還在本機可反查。如此自建 profile 即使日後在本機直接套用（mod 事後被移除）也能自動下載缺的，不必繞匯出／匯入。
- `handleExportProfile` 匯出時優先沿用 profile 既有的 `nexusSources`（建立時捕獲、最完整）；只有舊 profile（無來源）才即時反查。
