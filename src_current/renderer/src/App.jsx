...
export default function App() {
  ...
  const [pendingInstallPaths, setPendingInstallPaths] = useState(null);
  ...
  const handleExportProfile = (profileId) => {
    const profile = profiles.find(p => p.id === profileId);
    if (!profile) return;
    ...
    addToast(t.toastProfileExported || 'Profile exported', 'success');
  };
  ...
  const handleImportProfile = () => {
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        if (!imported.name || !imported.enabledModFilenames) throw new Error('Invalid profile');
        const newProfile = {
          ...imported,
          id: `profile-${Date.now()}`,
          createdAt: imported.createdAt || new Date().toISOString().split('T')[0],
        };
        const updated = [...profiles, newProfile];
        setProfiles(updated);
        persistSetting('profiles', updated);
        addToast(t.toastProfileImported || 'Profile imported', 'success');
      } catch {
        addToast(t.toastProfileImportFailed || 'Import failed', 'error');
      }
    };
    input.click();
  };
  ...
  const handleUrlInstall = async () => {
    if (!window.api || !urlInput.trim() || urlDownloading) return;
    setUrlDownloading(true);
    setUrlProgress(0);
    const unsub = window.api.mods.onDownloadProgress((p) => setUrlProgress(p));
    try {
      await window.api.mods.downloadUrl(urlInput.trim());
      addToast(t.toastUrlInstalled || 'Mod installed from URL', 'success');
      setUrlInput('');
    } catch {
      addToast(t.toastUrlFailed || 'Download failed', 'error');
    }
    unsub();
  ...
  // --- Install Preview ---
  const handlePreviewInstall = async (filePaths) => {
    if (!window.api) return;
    setPendingInstallPaths(filePaths);
    setPreviewModalOpen(true);
    setPreviewLoading(true);
    try {
      const results = await window.api.mods.preview(filePaths);
  ...
  const handleConfirmPreviewInstall = async () => {
    if (!window.api || !pendingInstallPaths) return;
    setPreviewModalOpen(false);
    try {
      await window.api.mods.install(pendingInstallPaths);
      addToast(t.toastInstalled, 'success');
    } catch (err) { console.error('Install failed:', err); }
    setPendingInstallPaths(null);
    setPreviewData(null);
  };

  // --- Backup / Restore ---
  const loadBackups = useCallback(async () => {
    if (!window.api?.mods?.listBackups) return;
    try { setBackups(await window.api.mods.listBackups()); } catch {}
  }, []);

  useEffect(() => { loadBackups(); }, [loadBackups]);

  const handleBackup = async () => {
    if (!window.api || backingUp) return;
    setBackingUp(true);
    try {
      await window.api.mods.backup();
      addToast(t.toastBackupDone || 'Backup complete', 'success');
      await loadBackups();
    } catch {
  ...
  const handleRestore = (backupPath) => {
    showConfirm(
      t.confirmRestoreTitle || 'Confirm Restore',
      t.confirmRestoreDesc || 'Restoring will overwrite current mods. Continue?',
      async () => {
        try {
          await window.api.mods.restoreBackup(backupPath);
          addToast(t.toastRestoreDone || 'Mods restored', 'success');
          await refreshMods();
        } catch {
  ...
  const guardGameRunning = useCallback((action) => {
    if (!isGameRunning) { action(); return; }
    showConfirm(
      t.gameRunning || 'Game Running',
      t.gameRunningWarning || 'The game is running. Modifying mods may cause issues. Continue?',
      () => { action(); closeConfirm(); }
    );
  }, [isGameRunning, showConfirm, closeConfirm, t]);
  ...
  return (
    <div className={`min-h-screen font-sans overflow-hidden flex relative transition-colors duration-700 ease-in-out ${isDark ? 'dark text-slate-200' : 'text-slate-800'}`}>
        ...
        <main className="w-full max-w-6xl flex-1 relative z-10 pb-12">
          ...
          {activeTab === 'settings' && (
            <SettingsTab
              ...
              handleBackup={handleBackup} handleRestore={handleRestore}
              backups={backups} backingUp={backingUp}
            />
          )}
      ...
      <PreviewModal
        ...
        onClose={() => { setPreviewModalOpen(false); setPendingInstallPaths(null); setPreviewData(null); }}
        previews={previewData}
        loading={previewLoading}
        onConfirm={handleConfirmPreviewInstall}
        t={t}
      />
    </div>
  );
