...
>
      ...
      <div className="flex items-center gap-2 px-2">
        <div className="flex-1 flex items-center gap-2 bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-full py-2 px-4 shadow-sm transition-all duration-500 focus-within:border-[var(--accent-400)] focus-within:shadow-md">
          <Link className="w-4 h-4 text-slate-400 dark:text-slate-500 shrink-0" />
          <input
            type="text"
            value={urlInput}
            onChange={(e) => setUrlInput(e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && urlInput.trim() && handleUrlInstall()}
            placeholder={t.urlPlaceholder}
            disabled={urlDownloading}
            className="flex-1 bg-transparent text-sm text-slate-700 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none font-medium"
          />
          {urlDownloading && (
            <div className="flex items-center gap-2">
              <div className="w-16 h-1.5 bg-slate-200/80 dark:bg-slate-800/80 rounded-full overflow-hidden">
                <div className="h-full rounded-full transition-all duration-300" style={{ width: `${urlProgress}%`, background: 'linear-gradient(to right, var(--accent-400), var(--accent-500))' }} />
              </div>
              <span className="text-[10px] font-bold tabular-nums" style={{ color: 'var(--accent-500)' }}>{Math.round(urlProgress)}%</span>
            </div>
          )}
        </div>
        <button
          ...
          className="px-4 py-2.5 text-xs font-bold rounded-full text-white transition-all duration-300 active:scale-95 shadow-sm disabled:opacity-40 disabled:cursor-not-allowed whitespace-nowrap"
          style={{ backgroundColor: 'var(--accent-500)' }}
          onMouseEnter={e => { if (!e.currentTarget.disabled) { e.currentTarget.style.backgroundColor = 'var(--accent-600)'; e.currentTarget.style.boxShadow = '0 10px 15px -3px rgba(var(--accent-rgb), 0.3)'; }}}
