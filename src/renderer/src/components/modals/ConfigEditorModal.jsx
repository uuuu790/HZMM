...
import { X, FileText, Save, RotateCcw, Sliders, RefreshCw, FolderOpen, ExternalLink } from 'lucide-react';
...
// Routes the click through window.api.mods.openSchemaPath — the main process
// resolves the path relative to gamePath / the mod folder and performs the
// shell action (reveal in explorer or open with default app).
function OpenPathButton({ modFilename, spec, addToast }) {
...
function SchemaRenderer({ schema, entries, lang, onUpdateValue, modFilename, addToast }) {
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-slate-300/50 dark:[&::-webkit-scrollbar-thumb]:bg-slate-700/50 [&::-webkit-scrollbar-thumb]:rounded-full">
          {loading ? (
            ...
            <SchemaRenderer schema={schema} entries={entries} lang={_lang} onUpdateValue={updateValue} modFilename={mod?.filename} addToast={addToast} />
          ) : (
            renderCommentMode()
          )}
        </div>
